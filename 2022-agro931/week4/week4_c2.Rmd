...
\begin{align*}
F_t = \frac{1}{N_e} + (1 - \frac{1}{2N_w})F_{t-1}
\end{align*}
...
\begin{align*}
F_t = [\frac{1}{N_e} + (1 - \frac{1}{2N_w})F_{t-1}] (1 - \mu )^2
\end{align*}
...
- $H_{exp}$ is often reported to compare populations
...
- It reflects potential diversity and how much heterozygosity you would expect if the population were to randomly mate (no selection, migration, mutation, etc.).
...
- Informative when comparing populations
  ...
  - A population with fewer alleles may be prone to inbreeding, drift
  
  - A locus with fewer alleles compared to others may indicate it is selected for/against
...
- When you look at a nucleotide position of two random copies of a gene, Diversity Diversity ( $\pi$ ) is the probability they are different
...
- $x_i$ is the freq of the $i^{th}$ genotype
- $\pi_{ij}$ is the proportion of nucleotide differences between $i^{th}$ and $i^{th}$ genotype
